...
export default function Summary() {
  const [approachInput, setApproachInput] = useState('');
  const [approachTags, setApproachTags] = useState([]);
  const [goals, setGoals] = useState([{ id: 1 }]); // Start with one goal

  // ---- NEW: date state + refs
  const [fromDate, setFromDate] = useState(""); // display as DD/MM/YYYY
  const [toDate, setToDate] = useState("");
  const fromPickerRef = useRef(null); // hidden native date inputs
  ...
  const handleUnitSelect = (unit, type) => {
    if (type === 'current') { setCurrentUnit(unit); setShowCurrentDropdown(false); }
    else { setTargetUnit(unit); setShowTargetDropdown(false); }
  };
  const toggleCurrentDropdown = () => { setShowCurrentDropdown(!showCurrentDropdown); setShowTargetDropdown(false); };
  const toggleTargetDropdown = () => { setShowTargetDropdown(!showTargetDropdown); setShowCurrentDropdown(false); };

  // ---- NEW: helpers for native date picker
  const ymdToDmy = (v) => {
    // v like "2025-11-05" -> "05/11/2025"
    if (!v) return "";
    const [y, m, d] = v.split("-");
    return `${d}/${m}/${y}`;
  };
  const openFromPicker = () => {
    if (fromPickerRef.current) {
      if (fromPickerRef.current.showPicker) fromPickerRef.current.showPicker();
      else fromPickerRef.current.click();
    }
  };
  const openToPicker = () => {
    if (toPickerRef.current) {
      if (toPickerRef.current.showPicker) toPickerRef.current.showPicker();
  ...
  // Add new goal
  const addNewGoal = () => {
    const newGoalId = goals.length + 1;
    setGoals(prev => [...prev, { id: newGoalId }]);
  };
    ...
    <>
      <div className='w-full'>
        <div className='pt-[23px]'>
          ...
          <div className='mt-[15px]'>
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <div className="relative flex-1">
                    ...
                    <div className="flex py-[15px] pl-[19px] pr-[13px] border border-[#E1E6ED] rounded-[8px] bg-white">
                      ...
                      />
                      <button type="button" onClick={openFromPicker} aria-label="Open from date picker" className="cursor-pointer">
                        <Image src="/icons/hugeicons_calendar-03.svg" alt="calendar" width={20} height={20} />
                      </button>
                  ...
                  <div className="relative flex-1">
                    ...
                    <div className="flex py-[15px] pl-[19px] pr-[13px] border border-[#E1E6ED] rounded-[8px] bg-white">
                      <input
                        ...
                        aria-label="To date"
                      />
                      <button type="button" onClick={openToPicker} aria-label="Open to date picker" className="cursor-pointer">
                        <Image src="/icons/hugeicons_calendar-03.svg" alt="calendar" width={20} height={20} />
                      </button>
                      <input
                        ref={toPickerRef}
            ...
            {goals.map((goal, index) => (
              <div key={goal.id}>
                <div className='px-[9px] py-[5px] text-[#252525] text-[12px] leading-normal font-semibold tracking-[-0.24px]'>Goal {index + 1}</div>

                <div className='flex gap-[7px]'>
                  <div className="relative flex-1">
                    <input
                      id={`goalTitle-${goal.id}`}
                      type="text"
                      placeholder=" "
                      className="peer block w-full py-[15px] pl-[19px] pr-[13px] text-[14px] text-[#252525] bg-white border border-[#E1E6ED] rounded-[8px] outline-none placeholder-transparent focus:border-blue-600"
                    />
                    <label
                      htmlFor={`goalTitle-${goal.id}`}
                      className="pointer-events-none absolute left-[19px] bg-white px-2 text-[14px] text-[#9CA3AF]
                       transition-all duration-200 ease-out
                       top-1/2 -translate-y-1/2
                       peer-focus:top-2 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:text-blue-600
                  ...
                  <div className="flex gap-10">
                    ...
                    <div className="flex flex-col" ref={currentDropdownRef}>
                      <div className="flex items-center py-[15px] pl-[19px] pr-[15px] border border-[#DA5747] rounded-[8px] bg-white relative">
                        ...
                        />
                        <div className="h-[20px] border-l border-[#E1E6ED] mx-3"></div>
                        <div className="flex items-center gap-2 cursor-pointer relative" onClick={toggleCurrentDropdown}>
                          <span className="text-[#252525] text-[12px] font-normal leading-[110%] tracking-[-0.24px]">
                            {currentUnit}
                          </span>
                          <IoIosArrowDown className={`text-[#A1A1A1] w-[15px] h-[15px] transition-transform ${showCurrentDropdown ? 'rotate-180' : ''}`} />
                    ...
                    {/* Target Stat with error and dropdown */}
                    <div className="flex flex-col" ref={targetDropdownRef}>
                      <div className="flex items-center py:[15px] py-[15px] pl-[19px] pr-[15px] border border-[#DA5747] rounded-[8px] bg-white relative">
                        <input
                          type="text"
                          placeholder="Target Stat"
                          className="w-full max-w-[90px] outline-none text-[#252525] text-[14px] font-normal leading-normal tracking-[-0.2px] placeholder:text-[#A1A1A1]"
                        />
                        <div className="h-[20px] border-l border-[#E1E6ED] mx-3"></div>
                        <div className="flex items-center gap-2 cursor-pointer relative" onClick={toggleTargetDropdown}>
                          <span className="text-[#252525] text-[12px]">{targetUnit}</span>
                          <IoIosArrowDown className={`text-[#A1A1A1] w-[15px] h-[15px] transition-transform ${showTargetDropdown ? 'rotate-180' : ''}`} />
                          {showTargetDropdown && (
                            <div className="absolute top-full right-0 mt-1 bg-white border border-[#E1E6ED] rounded-[8px] shadow-lg z-10 min-w-[100px] max-h-[200px] overflow-y-auto">
                              {unitOptions.map((unit) => (
        ...
        <div className='flex flex-col gap-[15px]'>
          <div className='px-5 py-[15px]'>
            <span 
              className='text-[#308BF9] font-semibold text-[12px] leading-normal tracking-[-0.24px] cursor-pointer'
              onClick={addNewGoal}
            >
              Add Another Goal
            </span>
          ...
          <div className='flex flex-col gap-3.5'>
            <div className='flex justify-between pr-[15px] items-center py-[15px] pl-5 border border-[#E1E6ED] rounded-[8px]'>
              ...
              />
              <IoIosArrowDown className="text-[#A1A1A1] w-[15px] h-[15px] cursor-pointer" onClick={addTag} title="Add" />
            </div>

            <div className="flex flex-wrap gap-2">
              {approachTags.map((tag, idx) => (
                <div key={`${tag}-${idx}`} className='flex items-center gap-2.5 px-5 py-2.5 rounded-[20px] border border-[#E48326] bg-[#FFF7F0] max-w-full'>
                  <span className='text-[#E48326] text-[12px] whitespace-nowrap font-semibold'>{tag}</span>
        ...
        <div className='py-[23px]'>
          <div className='flex gap-5 justify-end'>
            <div className='px-5 py-[15px] bg-white border border-[#D9D9D9] rounded-[10px]'>
              <span className='text-[#308BF9] text-[12px] font-semibold cursor-pointer'>Save as draft</span>
            </div>
            <div className='px-5 py-[15px] bg-[#308BF9] rounded-[10px]'>
              <span className='text-white text-[12px] font-semibold cursor-pointer'>Confirm & Next</span>
            </div>
          </div>
        </div>
      </div>
