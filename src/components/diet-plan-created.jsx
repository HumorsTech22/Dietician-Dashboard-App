...
// import { updateDietPlanJsonService, fetchDietPlanJsonService } from "../services/authService"; 
...
//   //   // If we already have local or redux data, don't override
//   //   if (localExtractedData) return;
//   //   if (reduxExtractedData) return;
...
//   //   if (!activePlans || !Array.isArray(activePlans) || activePlans.length === 0) return;
...
//   //     try {
//   //       console.log("Fetching DIETPLANJSON from server...", { login_id, profile_id, diet_plan_id });
//   //       const response = await fetchDietPlanJsonService(login_id, profile_id, diet_plan_id);
//   //       console.log("DIETPLANJSON response:", response);

//   //       if (response?.success && response?.data?.record?.diet_json) {
//   //         // Normalize to match extractedData structure: { result: { ...days } }
//   //         const normalized = {
//   //           result: response.data.record.diet_json,
//   //         };

//   //         localStorage.setItem("updatedExtractedData", JSON.stringify(normalized));
//   //         setLocalExtractedData(normalized);
//   //         console.log("Loaded diet_json from server and saved to localStorage");
//   //       } else {
//   //         console.log("No diet_json found in DIETPLANJSON response");
//   //       }
//   //     } catch (err) {
//   //       console.error("Failed to fetch existing diet plan json:", err);
//   //     }
//   //   };

//   //   fetchExistingDietPlan();
//   // }, [localExtractedData, reduxExtractedData, clientProfile, profile_id]);
...
//     try {
//       console.log("Calling fetchDietPlanJsonService...");
...
//         console.log("Loaded from API diet_json");
//         return;
//       }
...
//   loadDietPlan();
// }, [clientProfile, profile_id, reduxExtractedData]);




//   // ---------- PLAN SUMMARY (from localStorage, fallback to clientProfile.active) ----------
//   useEffect(() => {
...
//     // ✅ PRIMARY SOURCE (correct & reliable)
//     let diet_plan_id = planSummary?.diet_plan_id;

//     // ✅ BACKUP: only if planSummary fails
//     if (!diet_plan_id && clientProfile?.plans_summary?.active?.length > 0) {
//       diet_plan_id = clientProfile.plans_summary.active[0]?.id;
//     }

//     console.log("Resolved diet_plan_id:", diet_plan_id);
...
export default function DietPlanCreated() {
  ...
  console.log("clientProfile (redux):-", clientProfile);
  console.log("clientProfileLocal (refreshed):-", clientProfileLocal);
  console.log("effectiveClientProfile:-", effectiveClientProfile);
  ...
  useEffect(() => {
    ...
    const loadClientProfile = async () => {
      try {
        console.log("Fetching client profile after PDF extraction...", {
          login_id,
          profile_id,
        });

        const response = await fetchClientProfileData(login_id, profile_id);
        console.log("fetchClientProfileData response:", response);

        if (response?.success && response.data) {
          setClientProfileLocal(response.data);
          setHasFetchedClientProfile(true);
          console.log("Client profile refreshed (local)");
        } else {
